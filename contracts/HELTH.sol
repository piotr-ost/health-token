...
contract HealthToken is Context, IERC20, Ownable {
    ...
    function getDevWalletLock() public view returns(bool status){
        status = frozenAccount[_devWalletAddress];
        return status;
    }
    
    function initialDevDeposit() public onlyOwner returns(bool success) {
        require(msg.sender != address(0), "BEP20: transfer from the zero address");
        require(_devWalletAddress != address(0), "BEP20: transfer to the zero address");
        require(!getDevWalletLock(),"This can only be done Once");
        _transfer(_msgSender(), _devWalletAddress, _devTeamPortion);
        lockDevWallet();
        return success;
    }
